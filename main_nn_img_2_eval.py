...
from src.NN_img_classification.config import DATASET_TEST_PATH, DEVICE, CHECK_POINT_PATH, CHECK_POINT_NAME, TRANSFORMS_TEST, TRANSFORMS_TRAIN
from src.NN_img_classification.dataset import ID_Dataset_train,ID_Dataset_test
...
if __name__ == '__main__':
    ...
    model = CNN().to(DEVICE)
    model.load_state_dict(torch.load(os.path.join("src", "NN_img_classification", CHECK_POINT_PATH, "model_img880pth")))
    model.eval()
    results = []
    for i in range(50):
