...
from config import TRANSFORMS_TEST,\
                                             DEVICE, \
                                             CHECK_POINT_PATH



PATH_TO_EVAL = os.path.join("..","..","eval")
RESULT = os.path.join("..","..","results_eval","nn_classification.txt")

if __name__ == "__main__":
    data = []
    model = CNN().to(DEVICE)
    model.load_state_dict(torch.load(os.path.join( "final_model", "900model_img.pth")))
    model.eval()
